...
#Speed mode
def runSpeedMode(difficulty = 3 + 4):
    """
    ...
    while (time.time() - stateTime) < timeLimit:
        ...
        problemString, correctAnswersCounter = problem_generation.generateProblem('medium')
        print(f"Solve: {timeResponseValidity(problemString)}")
        ...
        # 3. Check the answer
        isCorrect = (userGuess == timeResponseValidity(problemString))
        if isCorrect:
            correctAnswers += 1
            ...
            print(f"Incorrect. The answer was {correctAnswers}. | Score: {currentScore}")
...
speedStats = runSpeedMode(difficulty = 3 + 4)
...
#Streak mode
def runStreakMode(difficulty = 3 + 4):
    """
...
#Time trackers
#Accuracy
def runAccuracyMode(difficulty = 3 + 4):
    """
    ...
    for i in range(totalProblems):
        # 1. Fetch the problem
        problemString, correctAnswers = problems.generateProblems('medium')
